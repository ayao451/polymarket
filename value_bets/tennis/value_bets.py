...
def main() -> int:
    parser = argparse.ArgumentParser(description="Polymarket Sports Betting Bot (Tennis)")
    ...
    run_totals_games = args.totals_games
    run_totals_sets = args.totals_sets

    if not (run_moneyline or run_spreads or run_totals_games or run_totals_sets):
        run_moneyline = True
        run_spreads = True
        run_totals_games = True
        run_totals_sets = True

    markets_to_run = {
        "moneyline": run_moneyline,
        "spreads": run_spreads,
        "totals": False,  # Tennis uses totals_games/totals_sets only
        "totals_games": run_totals_games,
        "totals_sets": run_totals_sets,
    }
    INNER_LOOP_DURATION_SECONDS = 30 * 60  # 30 minutes

    print("\n" + "=" * 80)
    print("POLYMARKET SPORTS BETTING BOT - STARTING UP (TENNIS)")
    print("=" * 80)
    ...
    print(f"  - Totals Games: {'YES' if markets_to_run['totals_games'] else 'NO'}")
    print(f"  - Totals Sets: {'YES' if markets_to_run['totals_sets'] else 'NO'}")
    
    if VERBOSE:
        print("\n[INIT] Creating bot interface...")
    bot = PolymarketSportsBettingBotInterface(sport="tennis", verbose=VERBOSE)
    if VERBOSE:
        print("[INIT] Bot interface created")
    
    if VERBOSE:
        print("\n[INIT] Creating Pinnacle odds service...")
    pinnacle = PinnacleTennisOddsService(timeout_ms=45000)
    if VERBOSE:
    ...
    # If test-date is provided, use it instead of today/tomorrow
    test_date = None
    if args.test_date:
        try:
            test_date = datetime.strptime(args.test_date, "%Y-%m-%d").date()
            print(f"\n[TEST MODE] Testing on date: {test_date}")
        except ValueError:
    ...
    while True:
        ...
        if VERBOSE:
            ...
            print(f"  -> Total: {len(polymarket_events)} Polymarket events (ATP/WTA only)")
        ...
        # Sort matched events: ATP prefixed first, then WTA, then everything else
        def _sort_key(event_tuple):
            event_slug = event_tuple[0]  # First element is always event_slug
            if event_slug.startswith("atp"):
                return (0, event_slug)  # ATP first
            elif event_slug.startswith("wta"):
                return (1, event_slug)  # WTA second
            else:
                return (2, event_slug)  # Everything else third
        ...
        # In test mode, only run one iteration
        inner_loop_start_time = time.time()
        max_iterations = 1 if test_date else None
        iteration_count = 0
        while True:
            iteration += 1
            iteration_count += 1
            ...
            if test_date and iteration_count > 1:
                if VERBOSE:
            ...
            for i, [event_slug, away_team, home_team, pinnacle_game] in enumerate(matched_events, 1):
                ...
                if VERBOSE:
                    print(
                        f"[OK] Found markets: moneyline={len(markets.get('moneyline', []))}, "
                        f"spreads={len(markets.get('spreads', []))}, "
                        f"totals_games={len(markets.get('totals_games', []))}, "
                        f"totals_sets={len(markets.get('totals_sets', []))}"
                    )
