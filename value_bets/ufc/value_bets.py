...
def main() -> int:
    parser = argparse.ArgumentParser(description="Polymarket Sports Betting Bot (UFC)")
    ...
    if not (run_moneyline or run_totals):
        run_moneyline = True
        run_totals = True
    ...
    print("\n" + "="*80)
    print("POLYMARKET SPORTS BETTING BOT - STARTING UP (UFC)")
    print("="*80)
    ...
    bot = PolymarketSportsBettingBotInterface(sport="ufc", verbose=VERBOSE)
    if VERBOSE:
        print("[INIT] Bot interface created")
    
    if VERBOSE:
        print("\n[INIT] Creating Pinnacle odds service...")
    pinnacle = PinnacleMMAOddsService(timeout_ms=45000)
    if VERBOSE:
    ...
    while True:
        ...
        if VERBOSE:
            ...
            print(f"  -> Total: {len(polymarket_events)} Polymarket events (UFC only)")
